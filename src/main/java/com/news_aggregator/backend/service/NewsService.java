    ...
    public void fetchAndSaveArticles(int targetNewArticles) {
        ...
        while (savedCount < targetNewArticles && page <= maxPages) {
            ...
            for (Object obj : list) {
                ...
                // 🔹 Assign Source (create if missing, handle duplicates safely)
Source source = null;
Object src = item.get("source");   // ✅ declare src here
...
if (src instanceof Map<?, ?> srcMap) {
    ...
    if (normalized != null && !normalized.isBlank()) {
        ...
        if (source == null) {
            try {
                ...
                System.out.println("🆕 Created new source: " + normalized);
            } catch (Exception e) {
                // Handle race-condition where another thread already inserted it
                source = sourceRepository.findByName(normalized).orElse(null);
                System.out.println("⚠️ Source already existed (race): " + normalized);
            }
        } else {
        ...
        if (source != null) {
            article.getSources().add(source);   // owning side
            source.getArticles().add(article);  // inverse side
        }
    }
}
                ...
                if (source == null) {
                    skippedCount++;
                    System.out.println("⏭️ Skipped (no valid source): " + article.getTitle());
                    continue;
                }
    ...
    return text.contains("esg")
            ...
            || text.contains("greenwashing") // ESG context
    ...
    return switch (name) {
        ...
        default -> name; // fallback: keep original
    };
}
